...
def get_daily_closing_prices(symbol):
    try:
        ...
        print(std_price)
        if symbol in list:
            cursor.execute("UPDATE data_bumaga SET std_price = %s WHERE name = %s",
                           (closing_prices, symbol))
        else:
            cursor.execute(
                "INSERT INTO data_bumaga(name, std_price) VALUES (%(name)s,, %(std_price)s)",
                {
                    "name": symbol,
                    "std_price": closing_prices
                }
            )
        return std_price
...
commands = [
    ...
    types.BotCommand(command='/np', description='Удалить ценную бумагу из портфеля')
]
...
class ManageStateGroup(StatesGroup):
    ...
    std_price_currency_name_state = State()
...
@dp.message_handler(commands=['std_price'])
async def add_currency_command(message: types.Message):
    await ManageStateGroup.std_price_currency_name_state.set()
    await message.reply("Введите имя ценной бумаги")

@dp.message_handler(state=ManageStateGroup.std_price_currency_name_state)
